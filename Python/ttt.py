...
class ttt:  # todo make a better class name?
    def __init__(self,
                 ...
                 optimizer: Optimizers = None, # todo figure out a good way to select parameters as well
    ...
    def validation_loader(self, loader: torch.utils.data.DataLoader):
        if not isinstance(loader, torch.utils.data.DataLoader):
            raise ValueError("validation_loader must be an instance of torch.utils.data.DataLoader")
    ...
    def validate(self):
        if self.validation_loader is None: raise ValueError("Cannot validate with no validation set")

    def test(self):
        if self.test_loader is None: raise ValueError("Cannot test with no test set")
